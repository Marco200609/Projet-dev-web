...
class EntrepriseM extends PdoM
{
    public function getEntreprises($page, $parpage, $nom = '', $ville = '') {
        ...
        $rq = $this->pdo->prepare("SELECT entreprise.nom, entreprise.logo, villes.nom_ville, AVG(note_entreprise.note)AS note, COUNT(offre.id_offre) AS nb_offre FROM entreprise
...
/*    public function getIdEntreprise($nom)
    ...
    {        $rq = $this->pdo->prepare("SELECT entreprise.nom, entreprise.logo, villes.nom_ville, AVG(note_entreprise.note)AS note, COUNT(offre.id_offre) AS nb_offre, entreprise.descriptif, entreprise.nb_employe, contact.email FROM entreprise
        ...
        $rq2 = $this->pdo->prepare("SELECT DISTINCT domaine
                                ...
                                ORDER BY date_offre DESC, id_offre DESC
                                LIMIT 3");
    ...
    public function setEntreprise($nom, $logo, $pays, $departement, $ville, $adresse, $mail, $telephone, $nb_employe, $description)
    {
        ...
        try {
            ...
            // 4. Adresse (toujours insérée car souvent unique)
            ...
            // 5. Contact (on vérifie si mail+tel existent déjà)
    ...
    public function getNote($id_entreprise, $id_user) {
        $rq = $this->pdo->prepare("SELECT note FROM note_entreprise WHERE id_entreprise_fk = :id_entreprise AND id_user_fk = :id_user");
